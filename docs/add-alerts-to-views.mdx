...
description: "Attach presence or absence alerts to a single view, or build reusable alert presets with custom schedules to apply across multiple views."
---
...
The Alerts feature has three cumulative permission levels. **Manage** includes everything in **Write**, and **Write** includes everything in **Read**.

- **Read**: open the alerts management page at **Settings** &gt; **Alerts** and view the list of alerts.
- **Write**: everything Read allows, plus add a preset, edit an alert, attach an alert or preset to a view, and unmute an alert.
- **Manage**: everything Write allows, plus delete a preset and detach an alert from a view.

If your permission level does not allow an action, its control appears disabled.

You assign these levels to roles at **Settings** &gt; **Organization** &gt; **Roles**. Click a role, then click the **Permissions** tab and set the level for **Alerts**. See [Role-Based Access Control](/docs/rbac) for details. If a control you need is disabled, contact an organization administrator to adjust your role's **Alerts** permission level.

## Add an Alert to a Single View

<Note>
Attaching an alert to a view requires the **Write** permission level. See [Permissions](#permissions).
</Note>
...
<Note>
Adding a preset requires the **Write** permission level. See [Permissions](#permissions).
</Note>
...
<Frame caption="A custom alert schedule showing Monday, Wednesday, and Friday from 8:00 AM - 5:00">
  ![Image](/images/docs/4osdlch1jol78630hbwli8qfxkllkfi6tyyviaic5gdpp3ox8hyax9ya2l54qlm2.png)
</Frame>

### Assign Preset to a View

<Note>
Attaching a preset to a view requires the **Write** permission level. See [Permissions](#permissions).
</Note>
