...
The Windows Security Template requires NXLog to be set up to collect security event logs. Uncomment the line `<Select Path='Security'>*</Select>` in your NXLog config file and restart NXLog to apply changes. The [NXLog](/docs/ingestion-integrations/nxlog-for-windows) ingestion integration topic contains more information on setting up NXLog. The Windows Security Template will not work with other integrations such as FluentD.
