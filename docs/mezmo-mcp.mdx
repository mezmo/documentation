...
These tools analyze OpenTelemetry trace data across your entire account, so you don't need to specify a source or component.
...
- `get_trace_spans`: Gets the spans for a given `trace_id`.
- `find_degraded_service_edges`: Detects service call edges with degraded latency by comparing the current period to a baseline period. Returns edges where the selected percentile metric has increased beyond the threshold you provide. Use it to spot service-to-service calls that are changing behavior.
- `list_service_latency_summaries`: Fetches bucketed latency summaries for specific operations between a service and its target. Each result is a timestamped bucket with latency metrics like `p50`, `p95`, and `p99`.
