...
Finds ranked log patterns (templates) in a time range against a baseline. By default it orders changed patterns first, then by severity, then deviation, then count. Deviation is a pattern's count divided by its baseline count, so it measures how far the pattern's volume moved from the baseline. A pattern counts as changed when it is novel with at least `min_count` events, or its deviation passes the service's change threshold.
...
- `min_count` (optional): Minimum pattern count to include. Defaults to `5`.
- `min_deviation` (optional): Minimum deviation to include.
- `min_sample_count` (optional): Minimum sample count to include. Defaults to `10`.
