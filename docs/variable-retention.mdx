...
description: "Create rules that apply custom retention periods to specific log lines and track their impact in the Usage Dashboard and email digests."
---

<Note>
This feature is only available for customers on an Enterprise plan. Access is governed by the Variable Retention permission levels described in the [Permissions](#permissions) section below. Please contact your Customer Support Manager or [support@mezmo.com](mailto:support@mezmo.com) for more information.
</Note>
...
Variable Retention has three cumulative permission levels. **Manage** includes everything in **Write**, and **Write** includes everything in **Read**.

- **Read**: open the **Variable Retention Rules** page and view the list of rules. You cannot make any changes at this level.
- **Write**: everything Read allows, plus add a rule, edit a rule, reorder rules or change a rule's priority, and enable or disable a rule.
- **Manage**: everything Write allows, plus delete a rule.

If your permission level does not allow an action, its control appears disabled on the **Variable Retention Rules** page.

You assign these levels to roles at **Settings** &gt; **Organization** &gt; **Roles**. Click a role, then click the **Permissions** tab and set the level for **Variable Retention**. See [Role-Based Access Control](/docs/rbac) for details.
...
This action requires the **Write** permission level. See [Permissions](#permissions).
...
This action requires the **Manage** permission level. See [Permissions](#permissions).
...
![](/images/docs/5fsdh5uko9ezbamiqxn2yypwrbr8b32lwmmut1ey8iyd0fj0x0tg419k6e8wfvwu.png)
...
![](/images/docs/p39gh7ntb1ozzfjvylkaba9hjtyrwbdb6yj6zzh67q1bwc75jlij7ys9sq5b97e4.png)
