...
description: "Learn how Mezmo roles and groups control member privileges and restrict access to log data, and when to use each."
---

Members are users who have signed up for your Organization. [Members](/docs/manage-access) can be assigned roles that give them certain privileges. Members can also be assigned to [groups](/docs/manage-groups) to give them access to certain logs.
...
Review the [Feature Access Matrix](/docs/feature-access-matrix) for a breakdown of each roles privileges.

## Groups

Groups contain `Members` who have access to logs based on a query. Only users with the role of `Member` or `Read-only` can be added to a group. `Owner and Admin` roles aren't able to be added to a group. By default members who aren't part of a group have access to everything. Learn more in [Manage Groups](/docs/manage-groups).
