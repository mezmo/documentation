...
| Trace ID field | The field to use as the identifier of a trace. | `.context.traceid` | 
| Rate | The rate at which to sample traces. For example, `10` means 1 out of every 10 traces will be sampled. | `10` | 
| Parent Span ID (Tail Only) | The field to use as the parent span identifier. | `.parent_spanid` | 
