...
| `aura.remote_agent_answer` | One remote agent's answer from a batched `ask_agent` call, emitted as soon as it lands (single-agent mode) | ✅ Implemented |
...
**Remote agent answer** (one per entry of a batched [`ask_agent`](/aura/a2a-remote-agents#asking-several-agents-at-once) call, as each answer lands):
```
event: aura.remote_agent_answer
data:
```
```json
{
...
Note: `remote` is the configured `[a2a.remote.<name>]` name. `text` is that entry's section of the tool result: the answer and its trailer, or the error when `success` is `false`. `elapsed_ms` is measured from the start of the batch. The event is emitted only for calls with two or more `calls` entries, only in single-agent mode, and only with `AURA_CUSTOM_EVENTS=true`. The final `aura.tool_complete` for the same call still carries the whole result.
