...
| `aura.usage` | Final token usage emitted at stream end (prompt/completion/total, plus optional cache read/creation sub-counts) | ✅ Implemented |
...
Two optional cache fields report prompt-cache activity:

- `cache_read_input_tokens` (integer, optional) — input tokens served from the provider's prompt cache.
- `cache_creation_input_tokens` (integer, optional) — input tokens written to the provider's prompt cache.

Both are sub-counts already included within `prompt_tokens`, not additive, and either or both may appear in a single response. Both are omitted entirely from the JSON when no provider reports cache usage, so existing consumers see identical payloads when caching is off. Even when caching is enabled, an individual response may omit them, for example on a cache miss or on the first request before any cache exists. Their absence on a given response does not indicate a problem.

For Anthropic and Bedrock agents, both fields can appear once `prompt_caching` is enabled (see the [Configuration Reference](/aura/configuration-reference)) and the provider reports cache activity. For OpenAI-backed agents, only `cache_read_input_tokens` can ever appear, because OpenAI caching is automatic and reports only cache reads. `cache_creation_input_tokens` therefore appears only for Anthropic and Bedrock. The same counts appear as span attributes on the [Tracing guide](/aura/tracing-spans).
