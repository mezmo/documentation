...
<Note>The `description` on each agent object is the agent's own summary. It is distinct from the `description` inside each `workers` entry, which describes an orchestration worker.</Note>
...
| `aura.context_usage` | Per-agent context-window occupancy derived from the provider's final-turn usage (not a local tokenizer) | ✅ Implemented |
...
`aura.usage` is cumulative provider-billed token accounting for cost: input plus output tokens summed across every LLM turn in the request. It reports these tokens identically in single-agent and orchestration modes. The payload carries only `CorrelationContext` (no `agent_id`). To compute context-window fill, use `aura.context_usage`.

**Context usage** (per-agent context-window occupancy, derived from the provider's final-turn usage):
```
...
`context_tokens` (u64) is the provider's final-turn input token count, and `response_tokens` (u64) is the provider's final-turn output token count. `context_window` (u64, optional) is the model's context-window limit; it is omitted when the limit is unknown or unset. The event also carries the shared `AgentContext` fields `agent_id` (required; `"main"` for single-agent) and `agent_name` (optional), plus the `CorrelationContext` fields `session_id` and `trace_id` (optional). `parent_agent_id` belongs to the `AgentContext` type, but `aura.context_usage` leaves it unset.

Note: Requires `AURA_CUSTOM_EVENTS=true`. `context_window` is omitted when the limit is unknown or unset, so treat its absence as unknown and fall back to `model_context_limit` from `aura.session_info` rather than reading it as zero. `aura.session_info` is emitted once at stream start and reports the default (coordinator) agent's `model_context_limit`; it carries no `agent_id`. In orchestration mode, a worker can run a model with a different context window (configured per `[orchestration.worker.<name>.llm]`). This fallback is reliable for the single-agent and coordinator case, but it may not match a worker's actual window.

Compute context-window fill as `(context_tokens + response_tokens) / context_window * 100`. The numerator sums `context_tokens` and `response_tokens` because the just-generated response tokens join the context for the next turn and count toward occupancy; this matches how the Aura CLI computes its "Context left" indicator. Use these two fields as the numerator, not `prompt_tokens` from `aura.usage`.

In single-agent mode the server emits exactly one `aura.context_usage` event, with `agent_id: "main"`. In orchestration mode it emits one event per worker task execution, with `agent_id` set to the worker name, so a worker that runs several tasks emits several events; it also emits a single `agent_id: "main"` reading for the conversation per turn. That reading is taken from the first LLM turn of the request's first planning call: the chat history plus the coordinator's planning prompt, with injected session history, as the provider counts it before any inner tool result lands. Later inner turns, continuation and planning cycles, and routing-correction attempts report nothing for `main`, because they carry scratch that is discarded with the turn. Unlike the `aura.orchestrator.*` events, which label the coordinator `agent_id: "coordinator"`, this event uses `agent_id: "main"` for the coordinator so clients track one conversation context across both modes. Because the coordinator reuses the single-agent `agent_id: "main"`, this event alone cannot tell a coordinator's occupancy apart from a single-agent session's. Resolve this from session context: if you have observed any `aura.orchestrator.*` event (or queried `/aura/info`) for the session, the session is orchestrated and an `agent_id: "main"` `aura.context_usage` event is the coordinator's; otherwise it is the single agent's.
...
| `agent_id` | Agent identifier (`main` for single-agent; `aura.context_usage` also uses `main` for the orchestration coordinator, see [Custom Event Types](#custom-event-types)) |
...
Note: `aura.context_usage` is likewise a base `aura.*` event (not orchestration-specific), including its per-mode emission counts and `agent_id` disambiguation; see the [Custom Event Types](#custom-event-types) section above.
