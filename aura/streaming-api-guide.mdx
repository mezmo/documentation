...
| `aura.orchestrator.task_blocked` | A gate blocked a worker tool call awaiting approval and parked it (emitted once per pending blocked call) |
| `aura.orchestrator.run_parked` | Orchestration run parked as a durable checkpoint after quiescence (terminal; emitted only after the checkpoint commit succeeds) |
...
The next two events fire when park mode is active. See [Park
mode](/aura/hitl#park-mode) for the no-resume constraint and how a parked run
differs from a parked approval.

**Task blocked** (a gated worker tool call was blocked and parked):
```
event: aura.orchestrator.task_blocked
data:
```
```json
{
...
`task_id`, `tool_call_id`, `tool_name`, `orchestrator_id`, and `worker_id` identify the blocked call. `decision_id` is the parked approval's id, the same one resolved via `POST /v1/approvals/{decision_id}`. `agent_id` and `session_id` are always present. `agent_name`, `parent_agent_id`, and `trace_id` are omitted when absent.

**Run parked** (blocked run persisted as a durable checkpoint):
```
event: aura.orchestrator.run_parked
data:
```
```json
{
  "run_id": "019edc27-d7e1-73d2-ac33-1a1e21b3fffd",
  "decision_ids": ["019edc27-e4d2-7950-abbf-e37a9060887d"],
...
- `decision_ids`: the decisions still awaiting a human decision. Serializes as `[]` when empty, never omitted.
- `expires_at` (RFC 3339): the deadline for recording the pending approval decisions (the decision window). It does not bound the checkpoint file's lifetime.
- `iteration` (1-indexed): the plan-execute iteration the run parked in.

Emitted only after the checkpoint commit succeeds, and not at all if the commit fails or no `memory_dir` is configured.

Both events require `AURA_CUSTOM_EVENTS=true` and orchestration active, like the other orchestration events on this page.
