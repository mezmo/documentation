...
```json
{
  ...
  "agents": [
    {
      "id": "orch",
      "instance_id": "6b7d9f24-8c31-5a0e-bf12-3d5e7a9c1b04",
      "model": "gpt-4o",
    ...
    {
      "id": "solo",
      "instance_id": "a1c3e5f7-2b48-5d6a-9e0f-1b3d5f7a9c2e",
      "model": "gpt-4o",
      "mcp_servers": {}
    }
  ]
...
Each agent object also carries an `instance_id`: the stable identity of that
running agent instance (see [Instance
identity](/aura/configuration-reference#instance-identity)). A current server
always resolves an environment seed — falling through to a random per-process
value when nothing else matches — so it computes and includes an `instance_id`
for every agent, not only when a config field is set. The field is absent only
from older servers that predate it, which is what keeps those servers and their
clients compatible.
