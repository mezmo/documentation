...
By default the [install script](https://raw.githubusercontent.com/mezmo/aura/main/scripts/install.sh) attempts to install from the AURA package repository through a native package manager, then falls back to Homebrew and finally a direct binary download. The script is configured entirely through the environment variables below.
  ...
  **Install script environment variables**
  ...
  | `AURA_INSTALL_METHOD` | How to install: `auto`, `homebrew`, `direct`, `deb`, or `rpm`, lowercase (for example `deb`, not `DEB`). The `deb` and `rpm` methods need a package manager that resolves dependencies: `apt-get` for `deb`; `dnf`, `microdnf`, `yum`, or `zypper` for `rpm`. Any other value is a fatal error. | `auto` |
  ...
  **Add the AURA repository**
  ...
  You need a package manager that can resolve and install dependencies, such as `dnf`, `microdnf`, `yum`, or `zypper`, plus root or `sudo` access. Bare `rpm` can't resolve dependencies, so it isn't enough on its own.
...
EOF
  ...
  The `aura` CLI and the daemon are independent packages, so you can install the CLI, the daemon, or both:
  ...
  Packages are verified against the repository's GPG signatures, and later upgrades come from `dnf update`. Individual `.rpm` files are also attached to each [GitHub release](https://github.com/mezmo/aura/releases), for mirroring into an internal repository.
  </Tab>
  ...
  **Add the AURA repository**
  ...
  You need root or `sudo` access, and `apt-get`. Bare `dpkg` can't resolve dependencies, so it isn't enough on its own.

  First, install the prerequisites and the repository signing key to `/usr/share/keyrings/mezmo-aura-archive-keyring.gpg`:
  ...
  ```bash
  sudo apt-get update
  sudo apt-get install aura aura-web-server
  ```
  ...
  Packages are verified through the `signed-by` keyring, and later upgrades come from `apt upgrade`. Individual `.deb` files are also attached to each [GitHub release](https://github.com/mezmo/aura/releases), for mirroring into an internal repository.
