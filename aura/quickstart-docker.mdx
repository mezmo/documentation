---
title: "Full-Stack Docker Quickstart"
description: "Run the whole AURA stack with docker compose — orchestrator mode, a chat UI, and a trace viewer."
---
...
<Note>Just want to talk to an agent? The [AURA Quickstart](/aura/quickstart) installs a binary and has you chatting in three commands, with no Docker and no clone. Use this page when you want the browser chat UI and the Phoenix trace viewer running alongside AURA.</Note>
...
<Note>A worker whose `mcp_filter` is *omitted* is granted **every** MCP tool. Set an explicit `mcp_filter` on each worker once an MCP server is configured; `mcp_filter = []` keeps a worker tool-free.</Note>
