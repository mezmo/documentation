...
The exporter supports two wire transports, `grpc` (the default) and
`http/protobuf`, which you select with `OTEL_EXPORTER_OTLP_PROTOCOL`. TLS is
enabled automatically when the endpoint uses an `https://` scheme. For the
full list of environment variables, see
[Standalone-mode OpenTelemetry](/aura/cli-reference#standalone-mode-opentelemetry).
