...
```json
{
  ...
  "instance_id": "9b2c4d6e-8a10-5f32-b7c9-1e3a5c7f9d2b",
  "scope": {
    "kind": "worker",
...
| `instance_id` | Identifies the running agent instance (pod, container, or process) that raised the request; stable across restarts. Its correlation value shows up across multi-instance deployments, where every instance carries a distinct value; in a single-instance deployment every request carries the same value. See [Instance identity](/aura/configuration-reference#instance-identity). |
