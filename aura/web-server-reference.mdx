...
The [CLI](/aura/cli-reference) starts an agent per terminal session, which suits interactive work. Running `aura-web-server` instead keeps agents resident behind an HTTP API, so a caller that isn't a person at a keyboard can start a run: an alert pipeline reacting to a firing monitor, a scheduled job, a CI step, or a chat client such as LibreChat or OpenWebUI.
...
- `metadata`: optional map of string to string. It is serialized into the `metadata` span attribute.

Both are request-only inputs: they surface only as span attributes on exported traces (`user.id` and `metadata` respectively) and are not part of the chat-completion response body.

See [Tracing & Span Layout](/aura/tracing-spans#span-attributes) for how these fields surface in traces.
...
OpenTelemetry and Phoenix environment variables (endpoint, project routing, and content recording) are not listed here; they are documented in [Tracing & Span Layout](/aura/tracing-spans) and the [AURA CLI Reference](/aura/cli-reference).
