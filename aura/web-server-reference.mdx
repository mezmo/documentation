...
<Note>**Active development** — APIs and configuration may change between releases as AURA evolves. [Issues and feature requests](https://github.com/mezmo/aura/issues) are welcome — we'd love your feedback.</Note>
...
<Note>The web server now starts through the `aura webserver` subcommand of the unified `aura` binary. The standalone `aura-web-server` executable is a deprecated compatibility shim: it prints a deprecation warning to stderr at startup and still starts the same server, so existing service definitions keep working. The warning is expected and harmless. Every server flag and environment variable is unchanged. The shim will be removed in a future release.</Note>

Migrating from the standalone binary? Replace `aura-web-server <flags>` with `aura webserver <flags>`. The flags and environment variables are identical.

If you installed a release binary (install script, Homebrew, or a deb/rpm package), start the server directly:

```bash
# Start the web server (installed binary)
aura webserver

# Custom configuration file
CONFIG_PATH=my-config.toml aura webserver

# A directory of configs (serves multiple agents)
CONFIG_PATH=configs/ aura webserver

# Custom host and port
HOST=0.0.0.0 PORT=3000 aura webserver
```

From a source checkout, build and run with `cargo`. The `--` separates cargo's own arguments from the arguments passed to the `aura` binary:
