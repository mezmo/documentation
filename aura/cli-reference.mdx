...
- **Approval gate display** — shows a pending-approval status line and the final outcome when a tool call is gated for human approval (see [Human-in-the-Loop Approval Gates](/aura/hitl#aura-cli-approval-display))
