...
| `OTEL_RECORD_CONTENT`                   | Gates content only on AURA-owned spans (`false` default omits it). Rig-owned chat/agent-turn/tool spans record content regardless, so `false` does not fully suppress exported content. See [Security and Data Handling](/aura/security). |
