...
1. Pick a server from the built-in catalog (Mezmo, PagerDuty, Datadog, Linear, or Kubernetes), or configure a custom server.
...
The wizard writes the `[mcp.servers.<name>]` block to your loaded agent's TOML config. This works whether that config was loaded from a single file or from a directory of configs.

When the config was loaded from a directory, the wizard resolves which agent's file receives the server. If exactly one agent is loaded, the wizard edits that agent's file regardless of any `/model` selection. If that selection names an agent that is not loaded, the wizard tells you before falling back to the only one it has. If several agents are loaded and your current `/model`-selected agent is one of them, the wizard uses that agent's file without prompting. If several are loaded and none matches your current selection, the wizard shows a numbered picker that lists every loaded agent by its id and filename, then asks which one should receive the server.

Hidden agents appear in the picker labeled `(hidden)` and stay selectable. The `hidden` setting governs only what `/model` and the server advertise, not which file the wizard may edit. See [Multiple Agents](/aura/configuration-reference#multiple-agents) for how `hidden` works.

If the target config file is a symlink, the wizard follows the link and edits the target, leaving the link intact. This keeps symlinked dotfile setups working.

Secrets go to a `.env` file next to your config, never into the TOML file, which uses `{{ env.VAR }}` placeholders. For orchestrated configs, the wizard can scope the new server's tools to specific workers through `mcp_filter`. You must restart AURA to activate a newly added server; there is no hot reload. The wizard supplements manual TOML editing rather than replacing it. See [`[mcp]`](/aura/configuration-reference#mcp) in the configuration reference for manual and advanced MCP setup, including the three transports. The scratchpad entry stays inert until agent-level scratchpad is enabled, and the wizard does not enable it for you. If your config lacks `[agent.scratchpad] enabled = true`, the wizard prints a note that interception is configured but inactive. To activate it, add `[agent.scratchpad] enabled = true`, a top-level `memory_dir`, and `[agent.llm].context_window` to your config, then see the [Scratchpad guide](/aura/scratchpad) for the full setup.
