...
```bash
# When --config is omitted, AURA searches a fixed order of locations for a config
aura
...
When you omit `--config`, the CLI resolves the agent config through an ordered search — see [Finding the agent config](#finding-the-agent-config).
...
```bash
aura init                 # interactive; asks where to save the config
aura init -o my.toml      # choose the output path
aura init --global        # write to the global agents directory
```
...
- **Choose a location**: it asks whether the config should live locally (`./config.toml`) or globally (`~/.aura/agents/<name>.toml`). Choosing global also asks for the agent name, which becomes both the filename and `[agent].name`, so a second global install lands beside the first rather than overwriting it. Skip the prompt with `--global` or `--output`.
- **Write** the config, plus a `.env` **only when you enter a new key** that isn't already in your environment. The `.env` goes in the same directory as the config. For a local config, the `.env` lands in your project directory, so add it to your `.gitignore`. For a global config, it lands in `~/.aura/agents/`, which sits outside any repo, so the `.gitignore` advice doesn't apply there. Every globally installed agent shares that one `~/.aura/agents/.env`, because each config references its key by env-var name (`{{ env.OPENAI_API_KEY }}`) and two agents on the same provider read the same variable.

<Note>When a later `aura init --global` supplies a different value for a variable already in `~/.aura/agents/.env`, it warns before replacing it, since replacing re-points every agent that references that variable. Pass `--api-key-env` to give the new agent its own variable instead. Agents for different providers use different variables (`OPENAI_API_KEY` versus `ANTHROPIC_API_KEY`), so this collision is uncommon.</Note>
...
| Flag | Description |
|------|-------------|
| `-o, --output <PATH>` | Output config path. If omitted, `aura init` prompts for a local or global location; non-interactive runs default to `./config.toml`. |
...
| `--global` | Write the config to the global agents directory (`~/.aura/agents/<name>.toml`) instead of prompting for a location. Conflicts with `--output`. |
...
A non-interactive run without `--global` always writes `./config.toml` and never depends on `$HOME`.

---

## Finding the agent config

This section is about where AURA looks for the *agent* config — the `config.toml` that defines your agents and their providers. That is a different file from the CLI preferences file (`cli.toml`) covered under [Configuration File](#configuration-file). The config discovery order matters most once you install global agents: a stray local `config.toml` can hide them.

When you omit `--config`, the CLI resolves the agent config by checking these locations in order, and the first match wins:

1. **`--config <path>` or the `AURA_CONFIG` env var** — a file or directory, used exactly as given. A path that does not exist is an error; the CLI does not fall through to a global config.
2. **`./config.toml`** — the current directory only. Parent directories are not searched.
3. **`~/.aura/agents/`** — every `*.toml` in the directory loads at once, so `/model` switches between all globally installed agents. Skipped when it holds no `.toml` file.
4. **`~/.aura/agent.toml`** — a single global config.

`~/.aura/config.toml` is deliberately not searched: that name is the legacy CLI preferences file (see the note under [Configuration File](#configuration-file)).

<Note>The first location that matches supplies **every** agent — the search does not merge locations. A local `./config.toml` therefore hides your global agents entirely, and they won't appear in `/model`. To use your global agents from such a directory, point the CLI at them explicitly with `--config`.</Note>

```bash
# Load the global agents even when a local ./config.toml exists
aura --config ~/.aura/agents/
```

When `--config` is absent, the CLI reads `AURA_CONFIG`, so exporting it has the same effect as passing the flag:

```bash
export AURA_CONFIG=~/.aura/agents/
aura
```

If none of these locations yields a config, the CLI exits and reports each location it searched, so you can see where to place the file.

A `.env` file beside the resolved config is loaded in addition to any `.env` in the current directory, so a global config can carry its own provider keys. When the resolved config is the `~/.aura/agents/` directory, that `.env` is the shared `~/.aura/agents/.env`.
...
| `AURA_CONFIG`                         | Path to the agent config file or directory to load, equivalent to `--config` (see [Finding the agent config](#finding-the-agent-config)). A path that does not exist is an error. | _(none)_                                 |
...
| `--config <PATH>`                          | `AURA_CONFIG`                        | Path to a TOML agent config file or directory (standalone mode). When omitted, the CLI searches for a config (see [Finding the agent config](#finding-the-agent-config)) |
| `--log-file <PATH>`                        | `AURA_LOG_FILE`                      | Append diagnostic tracing logs to this file. Omit for no logging (see [Logging](#logging))               |

**Precedence** (for settings also configurable in `cli.toml`, such as `--model`, `--log-file`, and `--enable-client-tools`): CLI flags > environment variables > project `cli.toml` > global `cli.toml` > defaults. Agent-config resolution (`--config` / `AURA_CONFIG`) follows its own search order — see [Finding the agent config](#finding-the-agent-config).

---

## Configuration File

This section covers the CLI *preferences* file (`cli.toml`), which is distinct from the agent `config.toml` that defines your agents — for that, see [Finding the agent config](#finding-the-agent-config). The CLI looks for a TOML preferences file in two places, with the project file
