...
`aura webserver` and the standalone CLI load `.env` automatically at startup, so a config generated here runs as-is.
...
filter mirrors the `aura webserver` verbose mode (info-level for aura crates
...
that `aura webserver` exports. See [Tracing & Span Layout](/aura/tracing-spans) for the full span reference.
