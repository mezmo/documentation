...
The wizard asks a few questions — including where to save the config — then writes a ready-to-run `config.toml`:
...
- **Output.** It asks where to save the config, defaulting to a local `config.toml` in the current directory, and writes a `.env` alongside it only when you entered a key that wasn't already in your environment. Choosing global lets an agent live outside any project directory — see [Finding the agent config](/aura/cli-reference#finding-the-agent-config) for how the CLI chooses between local and global configs.
...
### My global agents aren't showing up

A local `./config.toml` shadows your global agents, so they won't appear in `/model`. Reach them from that directory by pointing the CLI at them explicitly: `aura --config ~/.aura/agents/`. See [Finding the agent config](/aura/cli-reference#finding-the-agent-config) for the full search order.
