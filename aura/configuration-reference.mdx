...
When `memory_dir` is set, startup validation also checks that the directory is writable. The web server and the CLI create the configured path if it does not exist, then verify they can write to it. If the path cannot be created or written to, Aura exits with a permission error that names the configured path and the underlying operating-system error, before binding to any port or entering the REPL.
...
|---|---|---|---|
| `memory_dir` | string | — | Base directory for scratchpad storage and orchestration artifact persistence. Must be writable: Aura creates the path at startup if it doesn't exist, verifies it can write to the path, and exits if the path can't be created or written to. Required when scratchpad is enabled. |
