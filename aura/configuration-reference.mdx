...
Remote Aura agents this agent can call over A2A through the `ask_agent` tool. See [Calling Remote Agents](/aura/a2a-remote-agents) for how calls work, batching, and orchestration.

| Field | Type | Default | Description |
|---|---|---|---|
| `poll_interval_secs` | integer | `2` | Seconds between `GetTask` polls. Must be at least 1 and below the timeout. |
| `timeout_secs` | integer | `600` | Budget in seconds for one remote call, from send to answer. Must be at least 1. |
| `max_response_bytes` | integer | `65536` | Cap in bytes on the answer text given to the model. Minimum `1024`. |

### `[a2a.remote.<name>]`

One remote agent. `<name>` is how the model addresses it and may use only letters, digits, `_`, and `-`.

| Field | Type | Default | Description |
|---|---|---|---|
| `url` | string | — | **Required.** Absolute `http` or `https` origin of the remote Aura web server. Aura appends `/a2a/v1/rpc`. Credentials, a query string, or a fragment in the URL are refused. |
| `description` | string | — | What the remote agent can do, shown to the model in the `ask_agent` tool description. |
| `model` | string | — | Agent name or alias to select on the remote, sent as `x-aura-model`. |
| `headers` | table | `{}` | Static headers sent on every request to this remote. |
| `headers_from_request` | table | `{}` | Outgoing header name → incoming request header name, as in [MCP header forwarding](#header-forwarding-headers_from_request). A forwarded header takes precedence over a static one of the same name. |
| `poll_interval_secs` | integer | _(inherits `[a2a]`)_ | This remote's poll interval. |
| `timeout_secs` | integer | _(inherits `[a2a]`)_ | This remote's call budget. |

`x-aura-model`, `a2a-version`, `host`, `content-length`, and `content-type` are reserved and cannot appear in `headers` or `headers_from_request`.
...
[a2a.remote.stage]
url = "https://aura.stage.example.com"
description = "Verifies deployments in the stage cluster"
timeout_secs = 900
```

A single agent gets every remote. In orchestration mode, only workers that list remotes in [`remotes`](#orchestrationworkername) get `ask_agent`.
...
| `mcp_filter` | list of strings | — | Glob patterns selecting which MCP tools this worker can access. When omitted, all tools are included; an empty list `[]` grants no tools. Applies to MCP tools only, not to `ask_agent`. |
| `vector_stores` | list of strings | `[]` (none) | Names of vector stores this worker can access. Workers have no RAG access by default. |
| `remotes` | list of strings | `[]` (none) | Names of `[a2a.remote.<name>]` entries this worker can call through `ask_agent`. Workers have no remote access by default. Each name must be a configured remote. See [Calling Remote Agents](/aura/a2a-remote-agents#orchestration-per-worker-remotes). |
