...
Human-in-the-loop (HITL) approval gates let an agent ask for permission before running selected MCP tools. `[hitl]` is the enable bit: there is no separate `enabled` field. Adding the table turns HITL on, and `route` is required whenever `[hitl]` is present. See [HITL](/aura/hitl) for the full route contracts, SSE lifecycle events, current scope (single-agent vs. orchestration worker gating), and webhook signing.
...
Unlike the rest of `[hitl]`, webhook signing is configured by environment
variables rather than TOML. Signing is off by default; setting a secret enables
it.
...
refuses to start. HTTPS is required when signing is on.

See [HITL](/aura/hitl#sign-and-verify-approval-webhooks) for the header contract,
the responder signing obligation, and key rotation.
