...
| `user_agent` | string | `"aura/<version>"` | Identity AURA presents to every MCP server, as a `product/version` token. Sent verbatim as the HTTP `User-Agent` header on `http_streamable` and `sse` connections, and split into `clientInfo.name` and `clientInfo.version` in the MCP `initialize` handshake on every transport, including `stdio`. A token with no version part reports the running AURA version. Must be printable ASCII with a non-empty product part; the config fails to load otherwise. Set it when your MCP servers track clients and you want to tell one AURA deployment from another. A server entry can set its own `user_agent` to replace this for that server alone. |

```toml
[mcp]
sanitize_schemas = true
user_agent = "aura/0.2.15"  # default; override to tag a deployment, e.g. "aura-prod-us/0.2.15"
```
...
Every transport accepts an optional `user_agent` that replaces `[mcp].user_agent` for that server on both layers, the HTTP header and the handshake. Use it when one server expects a particular product token, or when you want one server to see a deployment tag that the others should not.

```toml
[mcp.servers.mezmo]
transport = "http_streamable"
url = "https://mcp.mezmo.com/mcp"
user_agent = "aura-prod-us/0.2.15"
```
