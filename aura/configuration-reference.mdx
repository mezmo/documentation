...
A `[orchestration.worker.<name>.scratchpad]` table replaces the agent-level `[agent.scratchpad]` table wholesale rather than merging, so fields the worker omits fall back to their own defaults, not to the agent-level values. Because `enabled` defaults to `false`, the worker table must repeat `enabled = true` even when the agent already enables scratchpad.
