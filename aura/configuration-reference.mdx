...
OpenAI caching is automatic, so there is no `prompt_caching` field. OpenAI cache reads surface in usage output as `cache_read_input_tokens`.
...
# prompt_caching = true  # opt-in prompt caching (off by default)
...
| `prompt_caching` | `bool` | `false` | Opt-in prompt caching. When enabled, Aura adds `cache_control` breakpoints on the system prompt and the last message. The tools definition renders before the system prompt, so those breakpoints also cover the tools; Aura adds no separate breakpoints for them. Off by default (cache writes bill at a premium, reads at a reduced rate). |

To verify caching and for the orchestration-worker override caveat, see [Confirm prompt caching](#confirm-prompt-caching) below.
...
context_window = 200000
# prompt_caching = true  # opt-in; model must support caching
```
...
| `prompt_caching` | `bool` | `false` | Opt-in prompt caching. When enabled, Aura adds `cachePoint` blocks after the system prompt, the tools, and the last message. Off by default (cache writes bill at a premium, reads at a reduced rate). The model must support caching or the request fails. Supported families include Claude 3.5 Sonnet v2, Claude 3.7, Claude 4.x, Claude 5, and Nova. |

#### Confirm prompt caching

Prompt caching reports the same way for Anthropic and Bedrock agents. To confirm caching is working, check the `cache_read_input_tokens` and `cache_creation_input_tokens` fields on the [`aura.usage` streaming event](/aura/streaming-api-guide) and the cache span attributes on the [Tracing guide](/aura/tracing-spans). Re-specify `prompt_caching` in an orchestration worker's LLM override when you enable it there, because a per-worker LLM override replaces the whole `llm` block. See [Per-Worker LLM Override](#per-worker-llm-override).
...
A worker with no `[orchestration.worker.<name>.llm]` block inherits `[agent.llm]`, including `prompt_caching`. A per-worker `[orchestration.worker.<name>.llm]` block replaces the entire LLM config (it is a complete config, not a partial patch, with no merging). You must re-specify `prompt_caching` in the override block, or it reverts to the default (`false`).
