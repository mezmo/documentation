...
The web server now runs as the `aura webserver` subcommand of the unified `aura` binary. See the [web server reference](/aura/web-server-reference) for details.
...
The Redis backend requires building with the `session-store-redis` cargo feature (`cargo build --release --features aura-web-server/session-store-redis`). The `aura-web-server` crate keeps its name, so the cargo feature path is unchanged even though you now run the server with `aura webserver`. The in-memory backend is always available. See [the session storage design doc](https://github.com/mezmo/aura/blob/main/docs/design/session-storage.md) for the design and Helm packaging roadmap.
